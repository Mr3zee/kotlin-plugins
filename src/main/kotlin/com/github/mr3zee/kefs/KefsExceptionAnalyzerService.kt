...
internal class KefsExceptionAnalyzerState : BaseState() {
...
internal class KefsExceptionAnalyzerService(
    private val project: Project,
    private val scope: CoroutineScope,
) : SimplePersistentStateComponent<KefsExceptionAnalyzerState>(KefsExceptionAnalyzerState()),
    Disposable {
    ...
    private fun startHandler() {
        ...
        if (handler.compareAndSet(null, exceptionHandler)) {
            ...
            project.service<KefsExceptionReporter>().start()
    ...
    fun updateState(enabled: Boolean, autoDisable: Boolean) = scope.launch(Dispatchers.EDT) {
        ...
        if (enabled) {
            ...
            rootLogger().removeHandler(handler)
            handler?.close()
            project.service<KefsExceptionReporter>().stop()
        }
    }
