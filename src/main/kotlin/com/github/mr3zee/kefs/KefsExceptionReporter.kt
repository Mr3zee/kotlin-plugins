...
internal class KefsExceptionReporterImpl(
    ...
    override fun stop() {
        ...
        project.service<KefsNotifications>().clear()
    ...
    private suspend fun processDiscovery(
        ...
        val plugin = project.service<KefsSettings>().pluginByName(discovery.pluginName) ?: return
        if (plugin.ignoreExceptions) {
            return
        }

        when (val result = KefsJarAnalyzer.analyze(discovery.jar)) {
            is KefsAnalyzedJar.Success -> {
    ...
    private fun matched(
        ...
        if (autoDisable) {
            ...
            project.service<KefsNotifications>().activate(ids)
            refreshNotifications()
        }
    }
    ...
    private inner class DiscoveryHandler : KotlinPluginDiscoveryUpdater {
        override suspend fun discoveredSync(
            ...
            if (new) {
                project.service<KefsNotifications>().deactivate(discovery.jarId)
