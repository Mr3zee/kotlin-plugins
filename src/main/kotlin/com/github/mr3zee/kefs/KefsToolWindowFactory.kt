...
internal class KefsToolWindowFactory : ToolWindowFactory, DumbAware {
    override fun init(toolWindow: ToolWindow) {
        toolWindow.title = KefsBundle.message("toolwindow.displayName")
        toolWindow.stripeTitle = KefsBundle.message("toolwindow.displayName")
    ...
    override fun createToolWindowContent(
        ...
        val state = KefsTreeState.getInstance(project)
        ...
        tabs.addTab(KefsBundle.message("tab.overview"), overviewPanel.overviewPanelComponent)
        tabs.addTab(KefsBundle.message("tab.logs"), createLogPanel())
    ...
    private fun createLogPanel(): JPanel {
        return JPanel().apply {
            ...
            val logNotice = GrayedLabel(KefsBundle.message("logs.tab.notice"))
    ...
    private fun createDiagnosticsPanel(
        project: Project,
        state: KefsTreeState,
    ): Pair<JComponent, KefsTree> {
        val panel = JPanel(BorderLayout())
        val settings = project.service<KefsSettings>()

        val tree = KefsTree(project, state, settings)
    ...
    private fun createDiagnosticsToolbar(tree: KefsTree): ActionToolbar {
        val group = DefaultActionGroup()

        group.add(object : AnAction(
            KefsBundle.message("toolbar.expandAll"),
            KefsBundle.message("toolbar.expandAll.description"),
        ...
        group.add(object : AnAction(
            KefsBundle.message("toolbar.collapseAll"),
            KefsBundle.message("toolbar.collapseAll.description"),
        ...
        group.add(object : KefsUpdateAction() {
            init {
                templatePresentation.text = KefsBundle.message("toolbar.update")
            }
        })

        group.add(object : KefsRefreshAction() {
            init {
                templatePresentation.text = KefsBundle.message("toolbar.refresh")
            }
        })
        ...
        group.add(object : ToggleAction(
            KefsBundle.message("toolbar.showSucceeded"),
            KefsBundle.message("toolbar.showSucceeded.description"),
        ...
        group.add(object : ToggleAction(
            KefsBundle.message("toolbar.showSkipped"),
            KefsBundle.message("toolbar.showSkipped.description"),
        ...
        group.add(object : KefsClearCachesAction() {
            init {
                templatePresentation.text = KefsBundle.message("action.clearCaches.dialog.title")
            }
        })

        val toolbar = ActionManager.getInstance()
            .createActionToolbar("KefsDiagnosticsToolbar", group, false)
    ...
    companion object {
        const val ID = "com.github.mr3zee.kefs.toolWindow"
        const val PROPORTION_KEY = "Kefs.Proportion"
    }
}

internal fun showKefsToolWindow(project: Project) {
    ToolWindowManager
        .getInstance(project)
        .getToolWindow(KefsToolWindowFactory.ID)
        ?.show()
}

internal class OverviewPanel(
    private val project: Project,
    private val state: KefsTreeState,
    private val tree: KefsTree,
) : Disposable {
        ...
        if (mavenId == null || partial.mavenId == null || mavenId == partial.mavenId) {
            if (requestedVersion == null || partial.requested == null || requestedVersion == partial.requested) {
                ...
                if (status is ArtifactStatus.ExceptionInRuntime && exceptionsRepaint != null) {
                    ...
                    project.service<KefsTreeStateService>().treeScope
    ...
    private fun render() {
        ...
        when {
            partial.pluginName == null -> {
                addRoot(GrayedLabel(KefsBundle.message("details.selectPlugin")), BorderLayout.CENTER)
            ...
            partial.requested == null -> {
                ...
                when (status ?: ArtifactStatus.InProgress) {
                        ...
                        if (analyzer.state.enabled) {
                            addRootPanel(customiseComponent = {
                                ...
                                if (status is ArtifactStatus.ExceptionInRuntime || status is ArtifactStatus.Success) {
                                    ...
                                    row {
                                        list = jbList(
                                            label = KefsBundle.message("analyzed.classes.label"),
                                    ...
                                    row {
                                        placeholder =
                                            grayed(KefsBundle.message("analyzed.classes.loading")).component
                                    }
                                    ...
                                    loadFqNamesAsync(jarId) { names ->
                                        ...
                                        project.service<KefsTreeStateService>().treeScope
                                            .launch(Dispatchers.EDT) {
                                                if (names.isEmpty()) {
                                                    placeholder?.text =
                                                        KefsBundle.message("analyzed.classes.none")
                            ...
                            addRoot(mainPanel { GridBagLayout() }).apply {
                                val label = GrayedLabel(KefsBundle.message("jar.analysis.disabled"))

                                val actionLink = ActionLink(KefsBundle.message("link.openSettings")) {
                                    KefsConfigurableUtil.showGeneral(project)
                                }
                    ...
                    is ArtifactStatus.FailedToFetch -> {
                        val sanitizedMessage = project.service<KefsStorage>()
                            .getFailedToFetchMessageFor(partial.pluginName, partial.mavenId, partial.requested)
                            ?: KefsBundle.message("error.unknown.reason")
                        ...
                        addRootPanel(
                            ...
                            row {
                                area = textArea()
                                    .label(KefsBundle.message("jar.load.failed.title"), LabelPosition.TOP)
                                    .align(AlignX.FILL)
                                    .rows(10)
                                    .applyToComponent {
                            ...
                            row {
                                checkBox(KefsBundle.message("label.softWrap")).applyToComponent {
    ...
    private fun openCacheLink(
        ...
        return if (isFile) {
            ActionLink(KefsBundle.message("link.showInCacheDir")) {
                revealFor(partial)
            }
        } else {
            ActionLink(KefsBundle.message("link.showCacheDir")) {
                revealFor(partial)
            }
        }.apply {
    ...
    private fun selectVersionPanel(): JPanel {
        return mainPanel { GridBagLayout() }.apply {
            val grayedLabel = GrayedLabel(KefsBundle.message("versions.select"))
    ...
    private fun inProgressPanel(type: NodeType): JPanel {
        return mainPanel { GridBagLayout() }.apply {
            val text = GrayedLabel(KefsBundle.message("panel.display.when.loaded", type.displayLowerCaseName))
    ...
    private fun skippedPanel(type: NodeType): JPanel {
        return mainPanel { GridBagLayout() }.apply {
            val text = GrayedLabel(KefsBundle.message("panel.not.requested", type.displayLowerCaseName))
            val description = GrayedLabel(KefsBundle.message("panel.resolution.lazy"))
    ...
    private fun disabledPanel(type: NodeType, pluginName: String): JPanel {
        return mainPanel { GridBagLayout() }.apply {
            val enableLink = ActionLink(KefsBundle.message("link.enableType", type.displayLowerCaseName)) {
                project.service<KefsSettings>().enablePlugin(pluginName)
            }

            val orLabel = GrayedLabel(KefsBundle.message("label.or"))

            val settingsLink = ActionLink(KefsBundle.message("link.openTheSettings")) {
                KefsConfigurableUtil.showArtifacts(project)
            }
    ...
    private fun partialSuccessPanel(type: NodeType): JPanel {
        return mainPanel { GridBagLayout() }.apply {
            ...
            val description = GrayedLabel(
                KefsBundle.message("partial.success.description")
            )
    ...
    private fun loadFqNamesAsync(jarId: JarId, onReady: (Set<String>) -> Unit) {
        ...
        project.service<KefsTreeStateService>().treeScope.launch(CoroutineName("load-fqnames-$jarId")) {
            val map = project.service<KefsExceptionReporter>().lookFor()
            val names = map[jarId].orEmpty()
            onReady(names)
        }
    }

    private fun revealFor(partial: PartialJarId) = project.service<KefsTreeStateService>().treeScope.launch {
    ...
    private fun exceptionTextAndHighlights(ex: Throwable, highlights: Set<String>): Pair<String, List<IntRange>> {
        ...
        if (lines.size > limit) {
            val footer = "\n" + KefsBundle.message("stacktrace.truncated", lines.size - limit)
            sb.append(footer)
        }
    ...
    private fun Panel.exceptionsPanel(
        ...
        if (status is ArtifactStatus.ExceptionInRuntime && report != null) {
            row {
                label(KefsBundle.message("exceptions.runtime.title"))
                    .comment(KefsBundle.message("exceptions.runtime.tipHtml"))
            }

            if (project.service<KefsSettings>().isEnabled(jarId.pluginName)) {
                row {
                    cell(ActionLink(KefsBundle.message("link.disableThisPlugin")) {
                        project.service<KefsNotifications>().deactivate(jarId.pluginName)
                        ...
                        project.service<KefsSettings>()
                            .disablePlugin(jarId.pluginName)
                    })

                    cell(ActionLink(KefsBundle.message("editor.notification.autoDisable")) {
                        project.service<KefsNotifications>().deactivate(jarId.pluginName)
                        // Refresh editor notifications on EDT via a project-bound scope
                        project.service<KefsTreeStateService>().treeScope
                            .launch(Dispatchers.EDT) {
                        ...
                        project.service<KefsExceptionAnalyzerService>()
            ...
            row {
                button(KefsBundle.message("button.createFailureReport")) {
                    loadingIcon?.isVisible = true
                    project.service<KefsTreeStateService>().treeScope.launch {
                        reportPath = project.service<KefsExceptionReporter>().createReportFile(report)
                ...
                }.comment(KefsBundle.message("button.createFailureReport.error"))
                    .apply {
                        button = component
                ...
                openReportAction = cell(ActionLink(KefsBundle.message("button.openReport")) {
                    project.service<KefsTreeStateService>().treeScope.launch {
            ...
            exceptionsPanel.add(com.intellij.ui.dsl.builder.panel {
                report.exceptions.forEachIndexed { index, ex ->
                    val groupTitle = KefsBundle.message(
            ...
            row {
                label(KefsBundle.message("exceptions.show.failed"))
                    .comment(KefsBundle.message("exceptions.show.failed.tip"))
            }
...
internal class KefsTreeState : BaseState() {
    ...
    companion object {
        fun getInstance(project: Project): KefsTreeState =
            project.service<KefsTreeStateService>().state
    }

    fun select(project: Project, partial: PartialJarId) {
        project.service<KefsTreeStateService>().treeScope.launch(Dispatchers.EDT) {
            try {
...
internal class KefsTreeStateService(
    val treeScope: CoroutineScope,
) : SimplePersistentStateComponent<KefsTreeState>(KefsTreeState())

internal class KefsTree(
    private val project: Project,
    val state: KefsTreeState,
    val settings: KefsSettings,
) : Tree(), Disposable {
    ...
    private fun reset() {
        ...
        redrawModel()
        project.service<KefsStorage>().requestStatuses()
    }
    ...
    inner class StatusUpdater : KotlinPluginStatusUpdater {
        override fun updatePlugin(pluginName: String, status: ArtifactStatus) = updateUi {
            this@KefsTree.updatePlugin(pluginName, status)
        }
        ...
        override fun updateArtifact(
            ...
            this@KefsTree.updateArtifact(pluginName, mavenId, status)
        ...
        override fun updateVersion(
            ...
            this@KefsTree.updateVersion(pluginName, mavenId, requestedVersion, status)
    ...
    private fun startUpdatingUi() {
        updaterJob = project.service<KefsTreeStateService>().treeScope
            .launch(Dispatchers.EDT + CoroutineName("KefsTreeUpdater")) {
                while (true) {
...
private fun statusToTooltip(type: NodeType, status: ArtifactStatus) = when (type) {
    NodeType.Plugin -> when (status) {
        is ArtifactStatus.Success -> KefsBundle.message("tooltip.plugin.success")
        is ArtifactStatus.PartialSuccess -> KefsBundle.message("tooltip.plugin.partial")
        ArtifactStatus.InProgress -> KefsBundle.message("tooltip.plugin.inProgress")
        is ArtifactStatus.FailedToFetch -> KefsBundle.message("tooltip.plugin.failed")
        is ArtifactStatus.ExceptionInRuntime -> KefsBundle.message("tooltip.plugin.exception")
        ArtifactStatus.Disabled -> KefsBundle.message("tooltip.plugin.disabled")
        ArtifactStatus.Skipped -> KefsBundle.message("tooltip.plugin.skipped")
    }

    NodeType.Artifact -> when (status) {
        is ArtifactStatus.Success -> KefsBundle.message("tooltip.artifact.success")
        is ArtifactStatus.PartialSuccess -> KefsBundle.message("tooltip.artifact.partial")
        ArtifactStatus.InProgress -> KefsBundle.message("tooltip.artifact.inProgress")
        is ArtifactStatus.FailedToFetch -> KefsBundle.message("tooltip.artifact.failed")
        is ArtifactStatus.ExceptionInRuntime -> KefsBundle.message("tooltip.artifact.exception")
        ArtifactStatus.Disabled -> KefsBundle.message("tooltip.artifact.disabled")
        ArtifactStatus.Skipped -> KefsBundle.message("tooltip.artifact.skipped")
    }

    NodeType.Version -> when (status) {
        is ArtifactStatus.Success -> {
            if (status.resolvedVersion.value == status.requestedVersion.value) {
                KefsBundle.message("tooltip.version.success.exact")
            } else {
                KefsBundle.message(
        ...
        is ArtifactStatus.PartialSuccess -> KefsBundle.message("tooltip.version.partial")
        ArtifactStatus.InProgress -> KefsBundle.message("tooltip.version.inProgress")
        is ArtifactStatus.FailedToFetch -> KefsBundle.message("tooltip.version.failed", status.shortMessage)
        is ArtifactStatus.ExceptionInRuntime -> KefsBundle.message("tooltip.version.exception")
        ArtifactStatus.Disabled -> KefsBundle.message("tooltip.version.disabled")
        ArtifactStatus.Skipped -> KefsBundle.message("tooltip.version.skipped")
    }
}
...
private fun KotlinPluginDescriptor.VersionMatching.toUi(): String {
    return when (this) {
        KotlinPluginDescriptor.VersionMatching.EXACT -> KefsBundle.message("version.matching.exact")
        KotlinPluginDescriptor.VersionMatching.SAME_MAJOR -> KefsBundle.message("version.matching.sameMajor")
        KotlinPluginDescriptor.VersionMatching.LATEST -> KefsBundle.message("version.matching.latest")
    }
}
...
private val NodeType.displayLowerCaseName
    get() = when (this) {
        NodeType.Plugin -> KefsBundle.message("nodeType.plugin")
        NodeType.Artifact -> KefsBundle.message("nodeType.artifact")
        NodeType.Version -> KefsBundle.message("nodeType.version")
    }
...
private fun ExceptionsReport.hint(): String {
    val analysis = when {
        kotlinVersionMismatch != null && isProbablyIncompatible ->
            KefsBundle.message("exceptions.analysis.header") +
                    KefsBundle.message(
    ...
    val action = when {
        reloadedSame && isLocal -> KefsBundle.message("exceptions.action.header") +
                KefsBundle.message("exceptions.action.local.reloaded.body")

        isLocal -> KefsBundle.message("exceptions.action.header") +
                KefsBundle.message("exceptions.action.local.body")

        reloadedSame -> KefsBundle.message("exceptions.action.header") +
                KefsBundle.message("exceptions.action.remote.reloaded.body")

        kotlinVersionMismatch != null -> KefsBundle.message("exceptions.action.header") +
                KefsBundle.message("exceptions.action.kotlinVersionMismatch.body")

        else -> KefsBundle.message("exceptions.action.header") +
                KefsBundle.message("exceptions.action.default.body")
    }
